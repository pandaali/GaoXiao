//
//  LastListTableViewController.swift
//  GaoXiao
//
...
class LastListTableViewController: UITableViewController, GADNativeExpressAdViewDelegate, GADVideoControllerDelegate {
    ...
    //列表广告
    let adPos = 3
    let adUnitId = "ca-app-pub-8687723258512783/7679839546"
    ...
    let adViewHeight = CGFloat(140)
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.tableView.register(UINib(nibName: "NativeExpressAd", bundle: nil),
    ...
    func loadNew() -> Void {
        ...
        //请求最新数据
        Article.getArticleNew(categoryid: categoryid, top:pageSize, refreshtime: strtime){ (contents) in
            if contents != nil{
                //网络请求是异步线程，需要加到主线程
                OperationQueue.main.addOperation {
                    ...
                    //当pos为4，则加载一条广告
                    var pos = 1
                    let adArticleRecord:ArticleRecord = ArticleRecord(article: Article(), url: "")
                    
                    for item in contents!.articles{
                        
                        //添加广告
                        if(pos == self.adPos){
                            self.articleList.insert(adArticleRecord, at: 0)
                            
                            self.tableView.layoutIfNeeded()
                            let adSize = GADAdSizeFromCGSize(CGSize(width: self.tableView.contentSize.width, height: self.adViewHeight))
                            ...
                            self.tableViewItems.insert(adView, at: 0)
                            self.adsToLoad.append(adView)
                            self.loadStateForAds[adView] = false
                        }
                        
                        self.articleList.insert(ArticleRecord(article: item, url: ""), at: 0)
                        self.tableViewItems.insert(ArticleRecord(article: item, url: ""), at: 0)
                ...
                if contents != nil{
                    ...
                    OperationQueue.main.addOperation {
                        ...
                        for item in contents!.articles{
                            ...
                            if(pos == self.adPos){
                                self.articleList.append(adArticleRecord)
                                ...
                                adView.rootViewController = self
                                adView.delegate = self
                                self.tableViewItems.append(adView)
                                self.adsToLoad.append(adView)
                                self.loadStateForAds[adView] = false
                            }
                            
                            self.articleList.append(ArticleRecord(article: item, url: ""))
                            self.tableViewItems.append(ArticleRecord(article: item, url: ""))
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case ImageType.ad:
            ...
            cell.nativeExpressAdView.adUnitID = adUnitId
            cell.nativeExpressAdView.rootViewController = self
            cell.nativeExpressAdView.delegate = self
            
            cell.nativeExpressAdView.load(GADRequest())
            //adsToLoad.append(cell.nativeExpressAdView)
            //loadStateForAds[cell.nativeExpressAdView] = false
